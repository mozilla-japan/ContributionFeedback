...
const FeedbackUI = Class({
  initialize: function initialize(options) {
    ...
    let createdDateText = created.getFullYear() +"-"+ month +"-"+ date +" "+ hours +":"+ minutes;
...
//    resolvedLabelContainer.setAttribute("flex", "1");
    ...
    if (relatedBugs.length == 0) {
      ...
      resolvedText = crashReport.title ? crashReport.title : crashReport.id;
      ...
      resolvedText = percent+"% RESOLVED";
    }

    updatedLabel.setAttribute("value", lastUpdated+" :");
    resolvedLabel.setAttribute("value", resolvedText);

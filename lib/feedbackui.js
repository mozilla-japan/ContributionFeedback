...
const FeedbackUI = Class({
  initialize: function initialize(options) {
    ...
    let createdDateText = created.getFullYear() +"."+ month +"."+ date +" "+ hours +":"+ minutes +":"+ seconds;
    let createdLabelText = createdDateText;
    ...
    if (relatedBugs.length == 0) {
      lastUpdated = createdDateText+" reported";
    } else {
      let totalResolved = 0;
      let imageBox = null;
      let userMap = {};
      for (let i = 0, n = relatedBugs.length, count = 0; i < n; i++) {
        ...
        if (relatedBug.lastUpdated && (lastUpdated.length == 0 || lastUpdated < relatedBug.lastUpdated)) {
      ...
      percent = parseInt((totalResolved / relatedBugs.length) * 100);
      ...
      lastUpdated = lastUpdated ? lastUpdated +" updated" : "LOADING:"+crashreport.id;
    }

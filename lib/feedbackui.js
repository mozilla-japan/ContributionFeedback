...
const FeedbackUI = Class({
  initialize: function initialize(options) {
    ...
    let createdDate = crashReport.createdDate;
    let month = getNumberLabel(createdDate.getMonth()+1);
    let date = getNumberLabel(createdDate.getDate());
    let hours = getNumberLabel(createdDate.getHours());
    let minutes = getNumberLabel(createdDate.getMinutes());
    let seconds = getNumberLabel(createdDate.getSeconds());
    let createdDateText = createdDate.getFullYear() +"-"+ month +"-"+ date;
    ...
    if (relatedBugs.length == 0) {
      ...
      let lastUpdatedDate = crashReport.lastUpdatedDate;
      ...
      if (percent == 100) {
        ...
        updatedLabel.className += thresholdDateTiem < lastUpdatedDate.getTime() ? " recently" : " progress";
      }
      container.style.backgroundSize = percent+"%";
      if (lastUpdatedDate) {
        container.setAttribute("tooltiptext", "LAST UPDATED:"+lastUpdatedDate+"\n"+createdLabelText);
        let month = getNumberLabel(lastUpdatedDate.getMonth()+1);
        let date = getNumberLabel(lastUpdatedDate.getDate());
        let lastUpdatedDateText = lastUpdatedDate.getFullYear() +"-"+ month +"-"+ date;
        updatedLabel.setAttribute("value", lastUpdatedDateText);
      } else {
        container.setAttribute("tooltiptext", createdLabelText);
